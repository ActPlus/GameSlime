...
import sk.actplus.slime.version2.Function;
import sk.actplus.slime.version2.MyMath;
...
public class Side extends Function {
    ...
    public static float getSlope(Vector2 A, Vector2 B) {
    ...
    public float getSlope(Vector2[] vertex) {
        return getSlope(vertex[0],vertex[1]);
    }
    ...
    private boolean isOnInterval(Vector2 point,boolean openInterval){
        Vector2 min = getDownLimit(new Vector2[]{A,B});
        Vector2 max = getUpLimit(new Vector2[]{A,B});
        if (openInterval) {
    ...
    public boolean isIntersecting(Side side, boolean openInterval) {
        Vector2 intersection = getIntersection(side);
        if(intersection!=null){
            if(contains(intersection,openInterval))
                return true;
        }
        return false;
    }

    public Vector2 getIntersection(Side side) {
